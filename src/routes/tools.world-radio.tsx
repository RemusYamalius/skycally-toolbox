...
function WorldRadioPage() {
  ...
  useEffect(() => {
    ...
    try {
      mapRef.current.flyTo(coords, Math.max(mapRef.current.getZoom(), 5), { duration: 0.8 });
    } catch {}
  }, [current]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const found = await apiFetch<Station[]>(
          `/stations/bycountrycodeexact/${country}?limit=1000&hidebroken=true&order=votes&reverse=true`,
        );
        if (!alive) return;
        setStations((prev) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const found = await apiFetch<Station[]>(
          `/stations/bytag/${encodeURIComponent(tag)}?limit=1000&hidebroken=true&order=votes&reverse=true`,
        );
        if (!alive) return;
        setStations((prev) => {
          const seen = new Set(prev.map((s) => s.stationuuid));
  ...
  return (
    <ToolPageShell
      title="World Radio"
      description="Explore live radio stations from every corner of the world on an interactive map."
    >
      <style>{`
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-[40fr_60fr] lg:grid-cols-[35fr_65fr] pb-28 max-w-full overflow-x-hidden">
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-border bg-card/50 p-3 md:order-1 order-2 min-w-0 md:h-[60vh] lg:h-[75vh]">
          <div className="flex gap-2 text-sm">
            <button
              onClick={() => setTab("all")}
              className={`flex-1 py-2 rounded-md border transition-colors ${tab === "all" ? "border-primary bg-primary/10 text-foreground" : "border-border text-muted-foreground hover:text-foreground"}`}
            >
              All Stations
            </button>
            <button
              onClick={() => setTab("fav")}
              className={`flex-1 py-2 rounded-md border transition-colors ${tab === "fav" ? "border-primary bg-primary/10 text-foreground" : "border-border text-muted-foreground hover:text-foreground"}`}
            >
          ...
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Search name, country, genre..."
            ...
            {visible.map((s) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <Heart
                      className={`w-4 h-4 ${isFav(s.stationuuid) ? "fill-current text-red-500" : "text-muted-foreground"}`}
                    />
                  </span>
                </button>
              );
            ...
            <div className="text-xs text-muted-foreground truncate flex items-center gap-2">
              {current && (
                <>
                  ...
                  {(current.tags ?? "").split(",")[0] && (
                    <span className="px-1.5 py-0.5 rounded bg-primary/10 text-foreground/80">
                      {(current.tags ?? "").split(",")[0]}
                    </span>
                  )}
                </>
      ...
      <AdZone id="world-radio-bottom" size="728x90" />
      ...
      <ToolSeoContent
        title="Free Online World Radio — Listen to Live Radio Stations Worldwide"
        description="Listen to thousands of live radio stations from every country worldwide. Free online radio player — search by country, genre, or name. No signup required."
        body={[
          "Skycally's World Radio lets you listen to thousands of live radio stations from every country in the world, directly in your browser. Search by country, genre, or station name — or browse the full directory and start streaming any station with a single click. No downloads, no accounts, and no interruptions.",
          "The station directory is powered by the Radio Browser API, a community-maintained database of over 30,000 radio stations worldwide. It covers news, music, talk radio, sports, classical, jazz, pop, rock, electronic, religious, and local community stations in dozens of languages across every continent.",
          "Stream quality varies by station — most broadcast at 128kbps MP3, though some offer higher quality AAC streams and others broadcast at lower bitrates for compatibility. The player shows the current bitrate, codec, and stream URL for each station. Some stations may be temporarily offline or geographically restricted.",
          "World Radio is a discovery tool as much as a player — it's a way to explore music and culture from countries you've never visited, practice languages by listening to native speakers, follow international news from local perspectives, or simply find a new favorite station you'd never encounter otherwise.",
        ]}
        faqs={[
          {
            question: "How many radio stations are available?",
            answer:
              "Over 30,000 live radio stations from countries worldwide, powered by the Radio Browser community database. Stations span news, music, sports, talk radio, and more in dozens of languages.",
          },
          {
            question: "Can I search by country or genre?",
            answer:
              "Yes. Filter stations by country, music genre, language, or search by station name. The directory updates automatically from the Radio Browser API.",
          },
          {
            question: "Why is a station not playing?",
            answer:
              "Stations occasionally go offline, change stream URLs, or restrict access by geographic region. Try refreshing the station list or selecting a different station.",
          },
          {
            question: "Does this work on mobile?",
            answer:
              "Yes. The radio player is fully responsive and works on smartphones and tablets. However, some mobile browsers require a user interaction before audio can start playing.",
          },
          {
            question: "Can I listen in the background?",
            answer:
              "Yes. Once a station starts playing, you can switch tabs or lock your screen and the audio continues. Use your device's media controls to pause or adjust volume.",
          },
          {
            question: "Is the audio free to stream?",
            answer:
              "Yes. All stations in the directory are publicly accessible internet radio streams. No subscription or payment is required.",
          },
          {
            question: "What audio formats are supported?",
            answer:
              "Most stations broadcast in MP3 or AAC format, which are supported by all modern browsers. A small number of stations use HLS streams, which may not work in all browsers.",
          },
          {
            question: "Can I add a radio station to the directory?",
            answer:
              "The station directory is maintained by the Radio Browser community. Visit radio-browser.info to submit new stations or report incorrect information.",
          },
        ]}
      />
