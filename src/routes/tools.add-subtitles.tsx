...
const COLOR_MAP: Record<string, string> = { white: "FFFFFF", yellow: "00FFFF", cyan: "FFFF00" };
...
function Page() {
  ...
  return (
    <ToolPageShell
      ...
      description="Burn subtitles into any video — works entirely in your browser."
    >
      <div
        ...
        <div className="mt-6 space-y-5">
          <div className="flex gap-3">
            {(["manual", "upload"] as const).map((m) => (
              <button
                key={m}
                onClick={() => setMode(m)}
                className={`flex-1 py-2.5 rounded-xl border text-sm font-medium transition ${mode === m ? "border-transparent bg-foreground text-background" : "border-border text-muted-foreground hover:text-foreground"}`}
              >
          ...
          {mode === "upload" ? (
            <div
              ...
              className="cursor-pointer rounded-2xl border-2 border-dashed border-border bg-card p-6 text-center hover:border-foreground/30"
            >
              <input
                ref={srtRef}
                type="file"
              ...
              />
              {srtFile ? (
                <p className="font-semibold">{srtFile.name}</p>
              ) : (
                <p className="text-muted-foreground">Upload .SRT subtitle file</p>
              )}
            </div>
          ) : (
              ...
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={() =>
                    setSubs((p) => [
                      ...p,
                      { id: crypto.randomUUID(), start: "00:00:00,000", end: "00:00:03,000", text: "New subtitle" },
                    ])
                  }
                ...
                <div key={s.id} className="rounded-xl bg-secondary/40 border border-border p-3 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      onChange={(e) => updateSub(s.id, { start: e.target.value })}
                      className="rounded-md border border-border bg-transparent px-2 py-1 text-xs font-mono"
                    />
                    <input
                      value={s.end}
                      placeholder="00:00:04,000"
                      onChange={(e) => updateSub(s.id, { end: e.target.value })}
                      className="rounded-md border border-border bg-transparent px-2 py-1 text-xs font-mono"
                    />
                  </div>
                  <input
              ...
              <div className="flex gap-2 mt-2">
                {(Object.keys(COLOR_MAP) as Array<keyof typeof COLOR_MAP>).map((c) => (
                  <button
                    key={c}
                    onClick={() => setColor(c)}
                    className={`px-3 py-1 rounded-lg text-xs capitalize border transition ${color === c ? "border-transparent bg-foreground text-background" : "border-border text-muted-foreground hover:text-foreground"}`}
                  >
                    {c}
                  </button>
                ))}
          ...
          >
            {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Captions className="w-4 h-4" />}{" "}
            {busy ? status || "Working..." : "Burn Subtitles"}
          </button>
          ...
          {result && (
            <div className="rounded-2xl border border-border bg-card p-5 flex flex-col items-center gap-4">
              ...
              <button
                onClick={() => downloadBlob(result.blob, "video_with_subtitles.webm")}
      ...
      <HowToUse
        steps={[
          ...
          "Write subtitles with custom timing, or upload a .SRT file. Choose font size and color.",
          "Click Burn Subtitles and download the video with permanent subtitles.",
        ]}
      />
      ...
      <ToolSeoContent
        title="Add Subtitles to Video Online Free — Burn SRT Subtitles, No Upload"
        description="Burn permanent subtitles into any video using Skycally's free online subtitle tool. Write subtitles manually or upload an SRT file. No software, no signup, no upload."
        body={[
        ...
        faqs={[
          ...
          {
            question: "Are the subtitles permanent?",
            answer:
          ...
          {
            question: "Can I add subtitles in Arabic or other RTL languages?",
            answer:
