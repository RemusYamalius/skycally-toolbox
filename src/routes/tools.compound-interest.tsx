...
function CompoundInterestPage() {
  ...
  const rows = useMemo(() => calcCompound(principal, rate, years, n, contrib), [principal, rate, years, n, contrib]);
  ...
  const returnPct = totalContributions > 0 ? ((last.balance - totalContributions) / totalContributions) * 100 : 0;
  ...
  const chartData = rows.filter(
    (_, i) => years <= 30 || (i + 1) % Math.ceil(years / 30) === 0 || i === rows.length - 1,
  );
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
        <div className="rounded-2xl border border-border bg-card p-5 space-y-5">
          ...
          <div>
            ...
            />
            <div className="mt-3">
              <Slider value={[Math.min(rate, 30)]} min={0} max={30} step={0.1} onValueChange={(v) => setRate(v[0])} />
            </div>
          </div>
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <StatCard label="Final Balance" value={fmt(last.balance)} accent="#06b6d4" />
            <StatCard
            ...
            />
            <StatCard label="Total Contributions" value={fmt(totalContributions)} accent="#7c3aed" />
            <StatCard
              label="Total Return"
              value={`${((last.balance / Math.max(1, principal) - 1) * 100).toFixed(1)}%`}
              sub="vs. initial investment only"
            />
          </div>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <XAxis dataKey="year" stroke="var(--muted-foreground)" fontSize={11} tickFormatter={(v) => `Y${v}`} />
                  <YAxis stroke="var(--muted-foreground)" fontSize={11} tickFormatter={(v) => fmt(v, true)} />
                  <Tooltip
                ...
                <tbody>
                  {rows.map((r, i) => (
                    <tr
                      ...
                      style={{
                        background: i % 2 === 0 ? "color-mix(in oklab, var(--muted) 30%, transparent)" : "transparent",
                      }}
                    >
